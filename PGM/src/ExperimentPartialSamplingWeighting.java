...
public class ExperimentPartialSamplingWeighting {
	...
	public static void main(String args[]) throws Exception {
		ExperimentPartialSamplingWeighting exp = new ExperimentPartialSamplingWeighting();
		
		exp.uaiFile = args[0];
		exp.evidFile = args[1];
		exp.poe = Double.parseDouble(args[2]);
		
		BufferedWriter bw = new BufferedWriter(new FileWriter(exp.uaiFile.substring(0, exp.uaiFile.lastIndexOf(".")) + "-result6.csv.log"));
			...
			for(int i = 0; i < exp.I.length; i++) {
				for(int n = 0; n < exp.N.length; n++) {
					...
					for(int itr = 0; itr < 10; itr++) {
						...
						double z = mgr.computePartialSamplingBasedPOE(exp.uaiFile, exp.evidFile, 5, exp.alpha[a], exp.I[i], exp.N[n]);

...
public class ExperimentSamplingWeighting {

	double alpha[] = {0, 0.02, 0.05, 0.2, 0.5, 0.75, 1};
	...
	public static void main(String args[]) throws Exception {
		ExperimentSamplingWeighting exp = new ExperimentSamplingWeighting();
		
		exp.uaiFile = args[0];
		exp.evidFile = args[1];
		exp.poe = Double.parseDouble(args[2]);
		
		BufferedWriter bw = new BufferedWriter(new FileWriter(exp.uaiFile.substring(0, exp.uaiFile.lastIndexOf(".")) + "-result5.csv.log"));
...
//						mgr.instantiateEvidence(P, exp.evidFile);
						double z = mgr.computeSamplingBasedPOEAdaptive(P, exp.evidFile, exp.alpha[a], exp.I[i], exp.N[n]);
